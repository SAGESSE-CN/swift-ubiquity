...
class ExampleTests: XCTestCase {
    ...
    override func setUp() {
        ...
        XPCDebugger.shared.on("api-forward") {
            $0.complete($0.parameter)
        }
        
        // In UI tests it is usually best to stop immediately when a failure occurs.
        continueAfterFailure = true

        // UI tests must launch the applicxation that they test. Doing this in setup will make sure it happens for each test method.
    ...
    func testBrowser() {
        ...
        let controllers = { (page: String, block: () -> ()) -> Void in
            ...
            block()
            if self.application.buttons["Browser"].exists {
                self.application.buttons["Browser"].tap()
            } else {
                self.application.buttons["Back"].tap()
            }
        }
