...
private class Cacher<T>: Caching<Change>, Change  {
    ...
    public func ub_changeDetails(forCollection collection: Collection) -> ChangeDetails? {
        ...
        let details = ref.ub_changeDetails(forCollection: collection)
    ...
    public func ub_changeDetails(forCollectionList collectionList: CollectionList) -> ChangeDetails? {
        ...
        // Make change details and cache
        let details = ref.ub_changeDetails(forCollectionList: collectionList)
...
extension Caching where T == Change {
    ...
    static func unwarp(_ value: Change) -> Change {
        if let value = value as? Cacher<Change> {
            return value.ref
        }
        return value
    }

    static func warp(_ value: Change) -> Change {
        if let value = value as? Cacher<Change> {
            return value
        }
        return Cacher<Change>(value)
    }
}
