...
internal class TilingViewLayout: NSObject {
    
    init(tilingView: TilingView) {
    ...
    func prepare() {
        ...
        var elements = [TilingViewLayoutAttributes]()
        var pages = [TilingViewLayoutPage]()
        var maps = [IndexPath: TilingViewLayoutAttributes](minimumCapacity: count)
        ...
        for section in 0 ..< tilingView.numberOfSections {
            for item in 0 ..< tilingView.numberOfItems(inSection: section) {
                ...
                let attributes = TilingViewLayoutAttributes(forCellWith: indexPath)
                ...
                if index == count || frame.maxX >= pageX + pageWidth {
                    let page = TilingViewLayoutPage(begin: pageStart, end: index)
    ...
    func invalidateLayout(at indexPaths: [IndexPath], _ sizeForItemWithHandler: (TilingViewLayoutAttributes) -> CGSize) {
        ...
        showedRect.origin.x = floor(tilingView.contentOffset.x / showedWidth) * showedWidth
    ...
    func layoutAttributesForElements(in rect: CGRect) -> [TilingViewLayoutAttributes]? {
        var begin: Int = .max
        var end: Int = .min
    ...
    private func _updatePage(_ page: TilingViewLayoutPage) {
...
//        page.end = end
    ...
    unowned var tilingView: TilingView
    
    private var _tilingViewContentSize: CGSize = .zero
    
    private var _version: Int = 0
    private lazy var _tilingViewLayoutPages: [TilingViewLayoutPage] = []
    private lazy var _tilingViewLayoutElements: [TilingViewLayoutAttributes] = []
    private lazy var _tilingViewLayoutElementMaps: [IndexPath: TilingViewLayoutAttributes] = [:]
}

fileprivate class TilingViewLayoutPage: NSObject {
